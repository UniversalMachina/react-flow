...
const EntityRelationshipNode = ({ data }) => {
  ...
  const handleAttributeChange = (id, newValue, field) => {
    setAttributes(attributes.map(attr => attr.id === id ? { ...attr, [field]: newValue } : attr));
  };
  ...
  {attributes.map((attribute, index) => (
    <div key={attribute.id} className="flex items-center p-2 bg-gray-600 border-t border-t-gray-300">
